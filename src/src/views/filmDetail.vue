  ...
  <div class="big">
    <div class="main">
      ...
      <div class="film-detail" :class="{ testT: flogT }">
        ...
        <div class="film-premiere-time grey-text">{{list.premiereAt | datefilter("xxxx年yy月xx日") }}上映</div>
        <div class="film-nation-runtime grey-text">
          {{ list.nation }} | {{ list.runtime }}分钟
        </div>
        <div :class="{ testT: flogT }" class="test grey-text" v-if="show1">{{ list.synopsis }}</div>
    ...
    <div class="photo">
      <div class="actors-title-bar second">
        ...
        <span class="all">全部({{ list.photos ? list.photos.length : ""}})></span>
      </div>
...
export default {
  ...
  computed: {
    url() {
      return this.urla + this.$store.state.urle + this.urlc;
    },
  },
  created() {
    let that = this;
