<template>
  <div class="big">
    <div class="main">
      ...
      <div class="film-detail" :class="{ testT: flogT }">
        <div class="col">
          <div class="film-name">
            <span class="name">{{ list.name }}</span>
            <span class="item">{{ list.filmType.name }}</span>
          </div>
        ...
        <div :class="{ testT: flogT }" class="test grey-text" v-if="show1">
          {{ list.synopsis }}
        </div>

        <div class="toggle" @click="flogT = !flogT">
          <img
            :class="{ imgJb: flogT }"
    ...
    <div class="photo">
      <div class="actors-title-bar second">
        ...
        <span class="all">全部({{ list.photos.length }})></span>
      </div>
    ...
    <div class="mainA" v-if="show">
      <div class="option">
        <div class="container">
          <div class="main-text">该影片目前没有排期，到首页看其他电影吧</div>
        </div>
...
export default {
  ...
  computed: {
    url() {
      return this.urla + this.$store.state.urlw + this.urlc;
    },
  },
  created() {
    let that = this;
    axios
      ...
      .then((res) => {
        ...
        that.show = res.data.data.film.isSale;
      });
  },
  methods: {
    fanhui() {
